...
我们部署了一个[在线网站](http://v8.ssr-fc.com/)可以实时上传 `V8` 内存快照结果并分析
...
目前默认的 `Http` 解析方案耗时在 `1-10s` 左右， `Wasm` 版本的方案解析时间在 `5-30s` 左右。我们将会在之后的版本中不断优化这一性能。
...
![](https://res.wx.qq.com/shop/public/8a4b4e3c-569c-4a3d-bed8-02b3325e2675.png)
...
点击左侧查看默认示例，我们将会通过上述代码的内存快照来生成图示。目前的计算时间根据内存快照的大小，大概需要 `1-10s` 左右的时间，目前的算法还有诸多可以优化的空间，我们将会在之后的版本中不断的优化性能。
