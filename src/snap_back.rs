pub mod snapshot {
    ...
    pub fn parse_snapshot(path: &str) -> Vec<RcNode> {
        ...
        set_flag2(4445, &node_map, &mut flag_marked_map, &mut flags);

        // get_child(1, &node_map, &mut has_marked_map); // 将一个节点的子节点插入到 has_marked_map 中，初始值为 false 代表还没到达
        mark_sweep(1, 23357, &node_map, &mut has_marked_map); // 把当前节点设置为不可到达后，标记从 gc roots 能到达的节点
    ...
    fn set_flag(
        ...
        flag_marked_map: &mut HashMap<usize, bool>,
        flags: &mut Vec<usize>,
    ) {
        ...
        flag_marked_map.insert(root_id, true);
        flags[root_id] = 1;
        root.edges.iter_mut().for_each(|edge| {
            ...
            {
                let to_node_id = usize::from(&edge.to_node);
                set_flag(to_node_id, node_map, flag_marked_map, flags);
            }
        });
    }
    fn set_flag2(
